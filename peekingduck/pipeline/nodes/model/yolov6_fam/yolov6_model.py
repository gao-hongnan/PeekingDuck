...
"""YOLOv6 models with model types: yolov6n."""
...
from peekingduck.pipeline.nodes.model.yolov6_fam.yolov6_files.detector import Detector
...
class YOLOv6Model(ThresholdCheckerMixin, WeightsDownloaderMixin):
    ...
    Attributes:
        ...
        detector (Detector): YOLv6 detector object to infer bboxes from a
    ...
    def __init__(self, config: Dict[str, Any]) -> None:
        ...
        # uncomment if not using LocalFileAdaptor method
        # model_dir = Path(
        #     "/Users/reighns/gaohn/gaohn_pkd/PeekingDuck/peekingduck_weights/yolov6/pytorch"
        # )
        model_dir = self.download_weights()

        # https://www.kianmeng.org/2021/11/w1514-using-open-without-explicitly.html
        # new pylint error since M1 Mac uses latest version of pylint
        with open(model_dir / self.weights["classes_file"], encoding="utf8") as infile:
            class_names = [line.strip() for line in infile.readlines()]

        self.detect_ids = self.config["detect"]  # change "detect_ids" to "detect"

        # cannot rearrange as it is positional arguments, consider dataclass?
